...
export default function RecallPage() {
  ...
  const handleCheck = () => {
    ...
    const isCorrect =
      input.trim().toLowerCase() === current.pronunciation?.toLowerCase() || input.trim().toLowerCase() === current.word?.toLowerCase();
  ...
  return (
    <div className="grid grid-cols-2 max-h-screen gap-10 p-10 text-white">
      <div className="flex-1 w-full bg-[#191c20] border border-[#2a2f36] rounded-xl p-8 flex flex-col gap-6 text-center">
        <p className=" text-[#9FB0C3]">What is the word for:</p>
            ...
            {history.map((item, i) => (
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm">
                    You: {item.userInput} → Correct: {item.word}({item.pronunciation})
                  </p>
                </div>
              </div>
            ))}
